...
struct RuntimeTuningState {
  float kpPitch=kCompetitionKpPitch,kdPitch=kCompetitionKdPitch,kpRoll=kCompetitionKpRoll,kdRoll=kCompetitionKdRoll;
  float kpHeight=kCompetitionKpHeight,kpYaw=kCompetitionKpYaw,kdYaw=kCompetitionKdYaw,targetHeightM=kCompetitionTargetHeightM;
  float leftMinDeg=kCompetitionLeftMinDeg,leftNeutralDeg=kCompetitionLeftNeutralDeg,leftMaxDeg=kCompetitionLeftMaxDeg;
  float rightMinDeg=kCompetitionRightMinDeg,rightNeutralDeg=kCompetitionRightNeutralDeg,rightMaxDeg=kCompetitionRightMaxDeg;
  float rearMinDeg=kCompetitionRearMinDeg,rearNeutralDeg=kCompetitionRearNeutralDeg,rearMaxDeg=kCompetitionRearMaxDeg;
} runtimeTuning;
production_control::Controller competitionController{}; production_control::CommandIngress competitionIngress{}; production_control::Output competitionOutput{}; production_control::StopReason latchedCompetitionReason=production_control::StopReason::None; uint32_t lastCompetitionOutputTxMs=0,competitionPwmWrites=0,competitionPwmErrors=0; uint64_t lastCompetitionActuatorUs=0,lastVescControlUs=0; uint16_t competitionLeftPulseUs=0,competitionRightPulseUs=0,competitionRearPulseUs=0; float vescTargetDuty=0.0f,vescAppliedDuty=0.0f; production_control::DutyRamp vescRamp(kVescMaxDuty,kVescRampRiseSeconds,kVescRampFallSeconds); production_control::ServoMapper competitionLeftServo(production_control::ServoTuning(servoAngleToPulseUs(kCompetitionLeftMinDeg),servoAngleToPulseUs(kCompetitionLeftNeutralDeg),servoAngleToPulseUs(kCompetitionLeftMaxDeg),kCompetitionLeftReversed)),competitionRightServo(production_control::ServoTuning(servoAngleToPulseUs(kCompetitionRightMinDeg),servoAngleToPulseUs(kCompetitionRightNeutralDeg),servoAngleToPulseUs(kCompetitionRightMaxDeg),kCompetitionRightReversed)),competitionRearServo(production_control::ServoTuning(servoAngleToPulseUs(kCompetitionRearMinDeg),servoAngleToPulseUs(kCompetitionRearNeutralDeg),servoAngleToPulseUs(kCompetitionRearMaxDeg),kCompetitionRearReversed)); production_control::ServoSpikeFilter competitionLeftSpikeFilter(kCompetitionServoSpikeThreshold),competitionRightSpikeFilter(kCompetitionServoSpikeThreshold);
