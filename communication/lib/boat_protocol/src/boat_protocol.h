...
struct __attribute__((packed)) SystemHealthPayload { uint64_t timestampUs; uint32_t flags; uint32_t imuAgeUs,tofAgeUs,gnssAgeUs,powerAgeUs,vescAgeUs; uint8_t safetyState,controlMode,stopReason,reserved; };
...
struct __attribute__((packed)) RuntimeTuningCommandPayload {
  uint8_t protocolVersion,reserved[3];
  uint32_t requestId,commandSequence;
  uint64_t sourceUs;
  float kpPitch,kdPitch,kpRoll,kdRoll,kpHeight,kpYaw,kdYaw,targetHeightM;
  float leftMinDeg,leftNeutralDeg,leftMaxDeg;
  float rightMinDeg,rightNeutralDeg,rightMaxDeg;
  float rearMinDeg,rearNeutralDeg,rearMaxDeg;
  uint32_t canonicalCrc;
};
struct __attribute__((packed)) ControlCommandAckPayload { uint32_t requestId,commandSequence; uint8_t commandType,disposition,safetyState,controlMode; uint16_t reason,reserved; uint64_t receivedUs,appliedUs; uint32_t canonicalCrc; };
static_assert(sizeof(ControlModeCommandPayload)==24,"ControlModeCommandPayload size"); static_assert(sizeof(ManualCommandPayload)==40,"ManualCommandPayload size"); static_assert(sizeof(HeadingTargetPayload)==28,"HeadingTargetPayload size"); static_assert(sizeof(RuntimeTuningCommandPayload)==92,"RuntimeTuningCommandPayload size"); static_assert(sizeof(ControlCommandAckPayload)==36,"ControlCommandAckPayload size"); static_assert(sizeof(ControlSnapshotPayload)==190,"ControlSnapshotPayload size"); static_assert(sizeof(InaStatusPayload)==32,"InaStatusPayload size"); static_assert(sizeof(VescTelemetryPayload)==48,"VescTelemetryPayload size"); static_assert(sizeof(ActuatorStatePayload)==36,"ActuatorStatePayload size"); static_assert(sizeof(SystemHealthPayload)==36,"SystemHealthPayload size"); static_assert(sizeof(WaypointSetPayload)==276,"WaypointSetPayload size"); static_assert(sizeof(WaypointAckPayload)==16,"WaypointAckPayload size");
...
enum class CalibrationKind:uint8_t{Static6Face=1,RotationX=2,RotationY=3,RotationZ=4,GyroBias=5,Magnetic=6,TimeOffset=7,Tof=8,ServoGeometry=9,VescTelemetry=10}; enum class CalibrationAction:uint8_t{Start=1,Stop=2}; struct __attribute__((packed)) CalibrationMarkerPayload{uint32_t sessionId;uint8_t kind,action,step,reserved;};
...
enum class P1CaptureAction:uint8_t{Start=1,Stop=2}; struct __attribute__((packed)) P1CapturePayload{uint32_t captureId;uint8_t action,reserved[3];}; struct __attribute__((packed)) P1CaptureAckPayload{uint32_t captureId;uint8_t action,status;uint16_t reserved;uint32_t firstSequence,lastSequence;};
